...
export class ReportesResumenComponent implements OnInit {
  ...
  filteredRecaudacion = computed(() => {
    const d = this.data();
    if (!d) return [];
    return this.filterByYear(d.recaudacionMensual, this.selectedYear());
  });

  filteredGastos = computed(() => {
    const d = this.data();
    if (!d) return [];
    return this.filterByYear(d.gastosMensuales, this.selectedYear());
  });
